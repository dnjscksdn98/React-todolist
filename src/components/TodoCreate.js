...
function TodoCreate() {
  ...
  const [value, setValue] = useState(""); // input state manager

  const dispatch = useTodoDispatch(); // bring dispatch context
  const nextId = useTodoNextId(); // bring next-id context

  const onToggle = () => {
    setOpen(!open);
  };

  // manage the value inserted
  const onChange = e => {
    setValue(e.target.value);
  };

  // if submit, it refresh page - so prevent default
  // add new todo
  const onSubmit = e => {
    e.preventDefault();
    dispatch({
